<template>
    <section id="journey" class="section section--journey" aria-labelledby="section-journey-title">
        <BaseEditorialMasthead class="journey__masthead" title-id="section-journey-title" :title="t('journey.title')"
            :kicker="t('journey.archiveLabel')" :stats="journeyStats" :eyebrow="t('nav.journey')"
            :description="t('journey.summary')" :figure="journeyRange" :status="t('journey.status')"
        ...
        <div class="journey__flow" data-animate>
            ...
            <JourneyViewRenderer :view-mode="viewMode" :affiliation-blocks="affiliationBlocks"
                ...
                :show-chapter-label="t('journey.showChapter')" :hide-chapter-label="t('journey.hideChapter')"
...
const journeyStats = computed(() => t('journey.stats')
    .replace('{chapters}', String(affiliationBlocks.value.length))
    .replace('{milestones}', String(milestoneCount.value)));
const journeyRange = computed(() => {
    const years = affiliationBlocks.value.flatMap((block) =>
        block.timeline.flatMap((item) => item.period.match(/\d{4}/g)?.map(Number) ?? []),
    );
